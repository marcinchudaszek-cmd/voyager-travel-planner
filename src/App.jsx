...
if(lat!=null&&lng!=null){fetch(`https://pl.wikipedia.org/w/api.php?action=query&list=geosearch&gscoord=${lat}%7C${lng}&gsradius=10000&gslimit=20&format=json&origin=*`).then(r=>r.json()).then(d=>{const g=(d&&d.query&&d.query.geosearch)||[];setPois(g.filter(x=>x.title&&x.title.trim()).map(x=>({name:x.title,dist:x.dist,lat:x.lat,lng:x.lon})))}).catch(()=>{})}
},[destination,lat,lng]);
...
{dtab==="pois"&&<div style={{padding:12,maxHeight:300,overflowY:"auto"}}>{pois.length>0?pois.map((x,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",background:i%2===0?(dark?"#242019":"#F4F2ED"):"transparent",borderRadius:8,marginBottom:4}}><div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:dark?"#F2EFE9":"#111111"}}>{x.name}</div>{x.dist!=null&&<div style={{fontSize:11,color:"#8A837C",marginTop:2}}>{x.dist<1000?`${Math.round(x.dist)} m stąd`:`${(x.dist/1000).toFixed(1)} km stąd`}</div>}</div>{onAddPlace&&<button onClick={()=>onAddPlace({id:uid(),name:x.name,description:"",address:"",priority:"medium",visited:false,lat:x.lat,lng:x.lng})} style={{background:"#3F6B4A",color:"#fff",border:"none",padding:"4px 10px",borderRadius:6,cursor:"pointer",fontSize:11,fontWeight:600,flexShrink:0,marginLeft:8}}>+ Dodaj</button>}</div>)):<div style={{textAlign:"center",padding:"20px 0",color:"#8A837C",fontSize:13}}>Brak atrakcji w pobliżu</div>}</div>}
</div>)}
...
if(!ready)return<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:400,color:"#8A837C"}}><div style={{textAlign:"center"}}><div style={{fontSize:48,marginBottom:8}}>🗺️</div>Ładowanie mapy...</div></div>;const geoCount=trip.places.filter(p=>p.lat!=null).length;
...
/* ═══ GROQ: model i diagnostyka błędów ═══
   Groq co jakiś czas wycofuje modele. Zamiast pokazywać wtedy
   "sprawdź klucz API", pytamy o listę dostępnych modeli i
   przełączamy się na działający. */
const MODEL_KEY="voyager-model";
const DEFAULT_MODEL="llama-3.3-70b-versatile";
function loadModel(){try{return localStorage.getItem(MODEL_KEY)||DEFAULT_MODEL}catch(e){return DEFAULT_MODEL}}
function clearModel(){try{localStorage.removeItem(MODEL_KEY)}catch(e){}}
...
const groqError=(res,data,model)=>{const m=data&&data.error&&data.error.message;
 if(res.ok)return `⚠️ Model „${model}" odpowiedział pustą treścią — prawdopodobnie nie nadaje się do rozmowy. Otwórz ⚙️ i kliknij „Zmień model".`;
 ...
 if(res.status>=500)return "⚠️ Serwery Groq nie odpowiadają. Spróbuj później — to nie problem z aplikacją.";
 return "⚠️ Błąd "+res.status+(m?": "+m:"")+" — jeśli się powtarza, pokaż mi ten komunikat."};
...
let mdl=loadModel();const doCall=async(limit)=>await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${apiKey}`},body:JSON.stringify({model:mdl,messages:[{role:"system",content:`Jesteś ekspertem od podróży, odpowiadasz WYŁĄCZNIE po polsku. Zasady formatowania:\n1. Lista miejsc/atrakcji: każde w OSOBNEJ LINII w formacie "Oficjalna nazwa — opis (cena lub godziny)"\n2. Plan dnia: każda linia "HH:MM — aktywność"\n3. Używaj wyłącznie OFICJALNYCH nazw geograficznych (np. "Colosseum", "Marienplatz", "Sagrada Família")\n4. Odpowiedź max 10 pozycji, bez wstępów i podsumowań\n5. Dodawaj emoji przed każdą pozycją\n6. Przy miejscach podaj praktyczną informację (cena biletu, godz. otwarcia, adres)\n${ctx}`},...history,{role:"user",content:txt}],temperature:0.7,max_tokens:limit||1400})});let res=await doCall(1400);
let data=await res.json().catch(()=>({}));
/* Model moze zwrocic 200 z pusta trescia: albo cala odpowiedz poszla
   w pole reasoning, albo limit tokenow skonczyl sie na "myśleniu".
   Wczesniej trafialo to do obslugi bledow jako absurdalny "Blad 200". */
const tresc=(d)=>{const w=d&&d.choices&&d.choices[0]&&d.choices[0].message;return String((w&&(w.content||w.reasoning))||"").trim()};
const powod=(d)=>d&&d.choices&&d.choices[0]&&d.choices[0].finish_reason;
let reply=tresc(data);
if(res.ok&&!reply&&powod(data)==="length"){res=await doCall(4000);data=await res.json().catch(()=>({}));reply=tresc(data)}
if(!reply&&(res.ok||isModelProblem(res,data))){
 const kandydaci=(await chatModels(apiKey)).filter(id=>id!==mdl).slice(0,5);
 for(const kand of kandydaci){mdl=kand;res=await doCall(2500);data=await res.json().catch(()=>({}));const t=tresc(data);
  if(res.ok&&t){saveModel(kand);reply=t;setMsgs(m=>[...m,{role:"assistant",text:`ℹ️ Poprzedni model nie odpowiadał poprawnie. Przełączyłem się na „${kand}".`}]);break}}}
if(reply)setMsgs(m=>[...m,{role:"assistant",text:reply}]);else setMsgs(m=>[...m,{role:"assistant",text:groqError(res,data,mdl)}])}catch(e){setMsgs(m=>[...m,{role:"assistant",text:"⚠️ Błąd: "+e.message}])}setLoading(false)};
...
{showSetup&&<div style={{padding:16,background:"#F4F2ED",borderBottom:"1px solid #E2DFD9"}}><p style={{fontSize:13,color:"#2B2825",marginBottom:8}}>🔑 Podaj klucz Groq API (darmowy):</p><a href="https://console.groq.com/keys" target="_blank" rel="noreferrer" style={{fontSize:12,color:"#3b82f6",fontWeight:600}}>→ Pobierz klucz na console.groq.com</a><div style={{display:"flex",gap:6,marginTop:8}}><input value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="gsk_..." style={{...s.aiI,flex:1}}/><button onClick={()=>{saveApiKey(apiKey);setShowSetup(false)}} style={{background:"#8E1F16",color:"#fff",border:"none",padding:"8px 16px",borderRadius:10,cursor:"pointer",fontSize:13,fontWeight:600}}>Zapisz</button></div><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginTop:10,fontSize:11,color:"#8A837C"}}><span>Model: <b style={{fontFamily:"'DM Mono',monospace",color:dark?"#D8D3C9":"#2B2825"}}>{loadModel()}</b></span><button onClick={()=>{clearModel();setMsgs(m=>[...m,{role:"assistant",text:"Model zresetowany. Przy następnym pytaniu dobiorę działający."}])}} style={{background:"none",border:`1px solid ${dark?"#2C2823":"#E2DFD9"}`,borderRadius:2,padding:"4px 10px",cursor:"pointer",fontSize:10,fontWeight:600,letterSpacing:".08em",textTransform:"uppercase",color:dark?"#D8D3C9":"#2B2825"}}>Zmień model</button></div></div>}
...
function ContentsSheet(){
  ...
  const items = [
    ...
    ["Mapa",`${trip.places.filter(p=>p.lat!=null).length} z ${trip.places.length} na mapie`],
